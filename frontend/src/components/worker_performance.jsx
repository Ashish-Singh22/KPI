          ...
          <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
              ...
              <p className="text-gray-600">
                ...
                {dateRange && (
                  <span className="ml-2 px-2 py-1 rounded-full bg-blue-100 text-blue-700 text-sm">
                    ({dateRange})
                  </span>
                )}
              </p>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    ...
                    {headers.map((h, idx) => (
                      <th
                        key={idx}
                        className="px-4 py-4 text-center text-sm font-bold text-gray-700 uppercase tracking-wider"
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {processedData.map((row, i) => {
                    ...
                    let vehicleUsed = "";
                    if (vehicleColumn && row.fullRow[vehicleColumn]) {
                      const vehicleData = row.fullRow[vehicleColumn];
                      if (Array.isArray(vehicleData) && vehicleData.length > 1) {
                        const [emp, vehicles] = vehicleData;
                        if (typeof vehicles === 'object' && vehicles !== null && !Array.isArray(vehicles)) {
                          // Handle object format like {HLOP: 63, RT: 2}
                          vehicleUsed = Object.entries(vehicles)
                            .map(([key, value]) => `${key}: ${value}`)
                            .join(", ");
                        } else if (Array.isArray(vehicles) || vehicles instanceof Set) {
                          vehicleUsed = Array.from(vehicles).join(", ");
                        } else {
                          vehicleUsed = String(vehicles);
                        }
                      } else if (typeof vehicleData === 'object' && vehicleData !== null) {
                        // Handle direct object format
                        vehicleUsed = Object.entries(vehicleData)
                          .map(([key, value]) => `${key}: ${value}`)
                          .join(", ");
                      } else {
                        vehicleUsed = String(vehicleData);
                      }
                    }
                    ...
                    return (
                      <tr key={i} className="hover:bg-gray-50 transition-colors">
                        <td
                          className="px-6 py-4 font-semibold text-gray-800 sticky left-0 bg-white z-10 border-r border-gray-200"
                        ...
                        {row.times.map((v, j) => (
                          <td key={j} className="px-4 py-4 text-center text-sm">
                            <span
                              className={`inline-flex items-center justify-center w-10 h-8 rounded-full text-sm font-semibold ${
                                v === 0
                        ...
                        {vehicleColumn && (
                          <td
                            className="px-4 py-4 text-center sticky right-0 bg-white z-10 border-l border-gray-200"
